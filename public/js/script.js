...
function sendOrderToWhatsApp(orderDetails) {
    ...
    const phoneNumber = '+5591992175347';//substituir pelo número que deseja enviar a mensagem 
...
function limparFucionario() {
    const email = document.getElementById('email').value = "";
    const senha = document.getElementById('senha').value = "";
}
function limparDeleta() {
    const email = document.getElementById('email').value = "";
}
...
document.getElementById('addFuncionarioForm').addEventListener('submit', function (event) {
    ...
    verificarEmailExistente(email).then(exists => {
        if (exists) {
            alert('Este e-mail já está em uso.');
        } else {
            fetch('/add-funcionario', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
...
document.getElementById('deleteFuncionarioForm').addEventListener('submit', function (event) {
    event.preventDefault();

    const email = document.getElementById('deleteEmail').value.trim();

    if (!email) {
        alert('Por favor, informe o email do funcionário a ser deletado.');
        return;
    }
    verificarEmailExistente(email).then(exists => {
        if (exists) {
            fetch('/delete-funcionario', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            alert('Esse Email não existe no banco de dados!');
            limparDeleta();
        }
    });
      ...
      .then(reservas => {
        for (let i = 1; i <= 5; i++) {
          ...
          mesaReservas.forEach(reserva => {
            ...
            excluirBtn.onclick = () => excluirReserva(id); 
            excluirBtn.onclick = () => {
